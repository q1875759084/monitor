...
## sendBeacon vs fetch：底层差异与 DevTools 表现

### DevTools 中的类型差异

用 `fetch` 发出的请求，Network 面板的「类型」列显示 `fetch`。  
用 `sendBeacon` 发出的请求，类型列固定显示 `ping`——Chrome 将其与 `<a ping>` 归为同一类，和请求内容无关，只取决于发送方式。

```
fetch / XHR    → 类型：fetch
sendBeacon     → 类型：ping（Chrome 固定分类）
```
...
```
JS 调用 fetch(url, options)
  → 创建 Request 对象（绑定到当前页面执行上下文）
  → 浏览器建立 HTTP 连接并发送请求
  → 等待响应，resolve Promise / 触发回调
```

关键：**请求生命周期与页面绑定**。页面关闭 → JS 执行上下文销毁 → 未完成的请求被强制中断。

**sendBeacon** 的工作方式：

```
JS 调用 navigator.sendBeacon(url, data)
  → 浏览器把数据放入独立的「信标队列」
  → 立即返回 true/false（同步返回，但网络发送是异步的）
  → JS 这边结束，浏览器在后台异步处理队列
  → 即使页面已关闭，浏览器进程仍会完成发送
```

"同步"指的是**返回值是同步的**（立即 true/false），但网络请求本身仍是异步的。返回 `true` 只代表"成功入队"，不代表"服务端已收到"。JS 在入队后就彻底失去了对这个请求的引用。

### 核心差异对比

| | fetch / XHR | sendBeacon |
|---|---|---|
| **返回值** | Promise（可拿到响应） | `boolean`（只知道有没有入队） |
| **能否读响应** | ✅ 能 | ❌ 不能 |
| **页面卸载时** | ❌ 请求被中断 | ✅ 浏览器保证送达 |
| **请求方式** | GET/POST/PUT/DELETE 均可 | 只能 POST |
| **数据大小** | 无限制 | 有队列大小限制（约 64KB） |
| **JS 控制权** | 完全控制（header、重试、取消） | 交给浏览器，JS 不再介入 |
| **设计语义** | "我要发一个请求，等结果" | "我要丢出去一条数据，不管结果" |

### sendBeacon 的缺陷

**1. 无法重试**

`sendBeacon` 没有响应回调，JS 拿不到 HTTP 状态码。网络波动导致的请求失败，代码层面完全无感知，无法实现重试逻辑。这是设计决策而非疏漏——信标的语义本身就是 fire-and-forget。

浏览器的 TCP 层会做传输层重传，但应用层的"请求失败 → 重新发送"做不到。

**2. 不能自定义 HTTP 方法，只支持 POST**

**3. 队列大小有限制（约 64KB）**

超限时 `sendBeacon` 返回 `false`，需要降级到 `fetch + keepalive`（reporter.ts 中已实现）。

**4. 无法中止**

一旦入队就无法取消，不像 `fetch` 可以通过 `AbortController` 中止。

### fetch keepalive:true 之后，sendBeacon 还剩什么优势

`keepalive: true` 让 fetch 也能在页面卸载后继续发送，两者看起来行为相近，但 sendBeacon 仍有几个真实优势：

**1. 独立预算，不和其他 keepalive 请求共享 64KB 上限**

`fetch keepalive` 的限制是：同一时刻所有 keepalive 请求的 body **总和**不超过 64KB（这是浏览器对"页面卸载后挂起请求"的全局预算）。如果页面卸载时同时有多个 SDK 发 keepalive 请求，总量超限就会失败。

`sendBeacon` 也有约 64KB 的限制，但走的是**独立的信标队列**，不和 keepalive 共享预算，更不容易踩到上限。

**2. 浏览器保证级别更高**

`keepalive: true` 是 Fetch API 的一个选项，"页面卸载后存活"依赖各浏览器的具体实现，移动端兼容性不完全一致。`sendBeacon` 是 W3C 专门为此场景设计的 API，浏览器对它的实现和测试更充分，语义保证更强。

**3. 不占用页面的 HTTP 并发连接数**

`fetch keepalive` 仍走正常的 HTTP 请求通道，受浏览器对同一域名 6 个并发连接的限制。`sendBeacon` 走独立的信标通道，不和页面的正常请求竞争并发槽。

**4. 语义清晰**

```javascript
navigator.sendBeacon(url, blob)       // 一眼看出：fire-and-forget，不关心响应
fetch(url, { keepalive: true, ... })  // 需要额外理解 keepalive 的含义
```
...
|---|---|---|
| 优势 | 可靠性高、独立预算、不占并发、语义明确 | 可拿响应、可重试、可自定义 method/header、可 AbortController 取消 |
| 适用场景 | 主路径：fire-and-forget 的监控上报 | 降级兜底：sendBeacon 队列满时 |

reporter.ts 里的策略 `sendBeacon 优先 → fetch keepalive 降级` 正是基于这个判断：优先用可靠性更强的 sendBeacon，队列满时降级到可控性更强的 fetch。
...
支付、订单必须重试 + 幂等，监控数据丢失只影响统计精度，可以容忍。这也是 reporter.ts 的 `catch` 静默处理的原因。

**需要可靠上报时的大厂方案（了解即可）**：

上报失败时写入 `localStorage` / `IndexedDB`，下次页面加载时补发。但这引入了去重问题（需要服务端幂等），实现成本高，只有专门的数据采集平台（神策、GrowingIO）才会实现。
